...
const fieldPreamble = ({ instancePath, parentSchema }, { fieldLabels }) => {
    switch (fieldLabels) {
        case 'js':
            ...
            return `the value at ${(0, util_1.jsonPath)(instancePath).replace(/^\$/, '')}`;
        case 'jsonPath':
            return `the value at ${(0, util_1.jsonPath)(instancePath)}`;
        case 'jsonPointer':
        case 'instancePath':
            if (instancePath === '') {
                return 'the root value';
...
const getMessage = ({ data, keyword, message, params, parentSchema, schemaPath, instancePath }, { fieldLabels }) => {
    const preamble = (0, exports.fieldPreamble)({
        instancePath,
        parentSchema
    }, { fieldLabels });
    ...
    switch (keyword) {
        case 'enum': {
            ...
            const allowed = (0, util_1.humanizeList)(list, 'or');
            return `${preamble} must be one of: ${allowed}`;
        }
        case 'type': {
            const list = Array.isArray(params.type) ? params.type : params.type.split(',');
            const expectType = (0, util_1.humanizeList)(list, 'or');
            const gotType = (0, util_1.humanizeTypeOf)(data);
            return `${preamble} must be ${(0, util_1.indefiniteArticle)(expectType)} ${expectType} but it was ${gotType}`;
        }
        case 'minLength': {
            const limit = params.limit;
            const charsLimit = (0, util_1.pluralize)('character', limit);
            const actual = data.length;
            const charsActual = (0, util_1.pluralize)('character', actual);
