...
describe("Wildcard prefix support", () => {
    ...
    it("wildcard with no defaultOrigin still blocks dangerous protocols", async () => {
        ...
        const tree2 = await processMarkdown("[Data](data:text/html,<script>alert('XSS')</script>)", {
            allowedLinkPrefixes: ["*"],
        });
        ...
        const tree5 = await processMarkdown("![Image](data:image/png;base64,iVBORw0KGg)", {
            allowedImagePrefixes: ["*"],
...
describe("Custom protocol support", () => {
    describe("allowedProtocols parameter", () => {
        ...
        it("allows vscode: protocol when specified in allowedProtocols", async () => {
            const tree = await processMarkdown("[Open in VS Code](vscode://file/path/to/file.ts)", {
...
[Postman](postman://open)
      `;
        ...
        it("handles custom protocols with complex paths and query params", async () => {
            const tree = await processMarkdown("[Complex](customapp://path/to/resource?param=value&other=123#section)", {
                allowedProtocols: ["customapp:"],
            });
            const link = findElement(tree, "a");
            expect(link).not.toBeNull();
            expect(link.properties.href).toBe("customapp://path/to/resource?param=value&other=123#section");
        });
